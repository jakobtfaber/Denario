...
def idea_prompt(topic):
    return [SystemMessage(content='''You are a scientist and your role is to generate a groundbreaking idea for a PhD student thesis.'''), HumanMessage(
...
def introduction_reflection(state):
    ...
    return [SystemMessage(content=f"""Your are a {state['writer']}"""), HumanMessage(
...
def refine_results_prompt(state):
    return [SystemMessage(content=f"""You are a {state['writer']}"""), HumanMessage(
        content=fr"""You are given the Results section of a paper that contains text and figures. The text and the figures were added independently, so there may not be a clear flow of integration between the two.
...
def clean_section_prompt(state, text):
    ...
    return [
        HumanMessage(
            content=fr"""You are given a section of LaTeX text. Your task is to make **minimal, clarity-focused edits** while ensuring the result is valid LaTeX and preserves the original meaning.
...
def fixer_prompt(text, section_name):
    ...
    return [
        HumanMessage(
            content=fr"""Given the text below, please extract all the text inside the {section_name} section.
...
def keyword_prompt(state):
    ...
    return [SystemMessage(content=f"""You are a {state['writer']}"""), HumanMessage(
        content=fr"""Given the idea and methods below, select a few AAS keywords from the list below. You must follow these rules:
